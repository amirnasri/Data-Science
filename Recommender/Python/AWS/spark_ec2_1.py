...
SPARK_HOME = '~/spark'
# Connect to the main and subordinates one-by-one and stop spark main
# and subordinate processes.
def stop_spark_cluster(main_nodes, subordinate_nodes, opts):
    main_address = get_dns_name(main_nodes[0])
    subordinate_addresses = [get_dns_name(i) for i in subordinate_nodes]
    command = os.path.join(SPARK_HOME, 'sbin/stop-main.sh')
    print_flush("Stoping spark on main node (%s)..." % main_address)
    _ssh_read(main_address, opts, command)

    command = os.path.join(SPARK_HOME, 'sbin/stop-subordinate.sh')
    for s in subordinate_addresses:
        print_flush("Stoping spark on subordinate node (%s)..." % s)
        _ssh_read(s, opts, command)


# Connect to the main and subordinates one-by-one and start spark main
# and subordinate processes.
def start_spark_cluster(main_nodes, subordinate_nodes, opts):
    stop_spark_cluster(main_nodes, subordinate_nodes, opts)
    main_address = get_dns_name(main_nodes[0])
    subordinate_addresses = [get_dns_name(i) for i in subordinate_nodes]
    command = os.path.join(SPARK_HOME, 'sbin/start-main.sh')
    print_flush("Starting spark on main node (%s)..." % main_address)
    _ssh_read(main_address, opts, command)

    command = os.path.join(SPARK_HOME, 'sbin/start-subordinate.sh') + ' spark://%s:7077' % main_address
    for s in subordinate_addresses:
        print_flush("Starting spark on subordinate node (%s)..." % s)
        _ssh_read(s, opts, command)

    with open("main_subordinate", "w") as f:
        f.write(main_address + "\n")
        for s in subordinate_addresses:
            f.write(s + "\n")

    print("Successfully started spark cluster")
    print("Main node:\n\t%s\n" % main_address)
    print("Subordinate node(s):")
    for s in subordinate_addresses:
        print("\t%s" % s)
    print
...
def setup_cluster(conn, main_nodes, subordinate_nodes, opts, deploy_ssh_key):
    ...
    start_spark_cluster(main_nodes, subordinate_nodes, opts)
    print("Done!")
